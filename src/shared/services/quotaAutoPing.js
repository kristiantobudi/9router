...
async function pingConnection(conn, provider, providerConfig, handler, deps, state = g) {
  ...
  // Quota exhausted → notify (rate-limited per connection) and skip pinging.
  if (isQuotaExhausted(quota) || (providerConfig.skipWhenBlockingQuotaExhausted && hasExhaustedBlockingQuota(quotas, providerConfig.quotaKey))) {
    const remaining = quota?.remaining;
    const used = quota?.used ?? quota?.usedAmount;
    const total = quota?.total ?? quota?.totalAmount;
    const { notifyEvent } = await import("@/shared/services/notifier");
    notifyEvent({
      type: "quota_exhausted",
      title: `${provider} quota exhausted`,
      message: `${connection.displayName || connection.name || connection.email || connection.id} — remaining: ${remaining ?? "0"}, used: ${used ?? "?"}${total != null ? ` / ${total}` : ""}${resetAt ? `, resets ${new Date(resetAt).toLocaleString()}` : ""}`,
      rateKey: `quota_exhausted:${provider}:${connection.id}`,
    }).catch(() => {});
    return;
  }
  ...
  const { notifyEvent } = await import("@/shared/services/notifier");
  notifyEvent({
    type: "quota_window_started",
    title: `${provider} quota window started`,
    message: `${connection.displayName || connection.name || connection.email || connection.id} — new window active until ${new Date(resetAt).toLocaleString()}`,
    rateKey: `quota_window:${provider}:${connection.id}:${normalizeResetKey(resetAt)}`,
  }).catch(() => {});
