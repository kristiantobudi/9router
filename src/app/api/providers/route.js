...
export async function POST(request) {
  try {
    ...
    const proxyPoolListInput = Array.isArray(body.proxyPoolIds) ? body.proxyPoolIds
      : (body.proxyPoolIds !== undefined ? [body.proxyPoolIds] : null);
    let proxyPoolIds = null;
    if (proxyPoolListInput !== null) {
      proxyPoolIds = [...new Set(proxyPoolListInput.map((x) => String(x).trim()).filter((x) => x && x !== "__none__"))];
      for (const id of proxyPoolIds) {
        const pool = await getProxyPoolById(id);
        if (!pool) {
          return NextResponse.json({ error: `Proxy pool not found: ${id}` }, { status: 400 });
        }
      }
    }
    ...
    if (proxyPoolIds !== null && proxyPoolIds.length > 0) {
      mergedProviderSpecificData.proxyPoolIds = proxyPoolIds;
      mergedProviderSpecificData.proxyPoolId = proxyPoolIds[0];
    }
