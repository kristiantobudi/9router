...
export default function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete, oneByOneStatus = null, autoPing = null }) {
  ...
  const boundPoolIds = connection.providerSpecificData?.proxyPoolIds?.length
  ...
  const firstPool = boundPoolIds.length > 0 ? proxyPoolMap.get(boundPoolIds[0]) : null;
  const hasLegacyProxy = connection.providerSpecificData?.connectionProxyEnabled === true && !!connection.providerSpecificData?.connectionProxyUrl;
  const hasAnyProxy = boundPoolIds.length > 0 || hasLegacyProxy;
  const proxyDisplayText = boundPoolIds.length > 0
    ? `Pools: ${boundPoolIds.map((id) => proxyPoolMap.get(id)?.name || `${id} (inactive/missing)`).join(", ")}`
    : hasLegacyProxy
  ...
  let maskedProxyUrl = "";
  if (firstPool?.proxyUrl || connection.providerSpecificData?.connectionProxyUrl) {
    const rawProxyUrl = firstPool?.proxyUrl || connection.providerSpecificData?.connectionProxyUrl;
    try {
  ...
  const noProxyText = firstPool?.noProxy || connection.providerSpecificData?.connectionNoProxy || "";

  let proxyBadgeVariant = "default";
  if (boundPoolIds.length > 0) {
    const allActive = boundPoolIds.every((id) => proxyPoolMap.get(id)?.isActive === true);
  ...
  // Multi-pool failover: draft selection in the dropdown, applied on save.
  const openProxyDropdown = () => {
    setDraftPoolIds(boundPoolIds);
  ...
  const handleApplyProxy = async (poolIds) => {
    setUpdatingProxy(true);
    try {
      await onUpdateProxy(poolIds);
    } finally {
        ...
        <div className="grid flex-1 grid-cols-3 gap-1 sm:flex sm:flex-none">
          ...
          {(proxyPools || []).length > 0 && (
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full z-50 mt-1 max-w-[78vw] min-w-[240px] rounded-lg border border-border bg-bg py-1 shadow-lg">
                  <div className="border-b border-border px-3 pb-1 pt-1 text-[10px] uppercase tracking-wide text-text-muted">
                    Failover pools — first alive wins
                  </div>
                  {(proxyPools || []).map((pool) => (
                    <label
                      key={pool.id}
                      className="flex w-full cursor-pointer items-center gap-2 px-3 py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/5"
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className={`truncate ${pool.isActive ? "text-text-main" : "text-text-muted line-through"}`}>
                        {pool.name}
                      </span>
                    </label>
                  ))}
                  <div className="mt-1 flex gap-1 border-t border-border px-2 py-1.5">
                    <button
                      onClick={() => handleApplyProxy([])}
                      className="flex-1 rounded px-2 py-1 text-xs text-text-muted hover:bg-black/5 dark:hover:bg-white/5"
                    >
                      None
                    </button>
                    <button
                      onClick={() => handleApplyProxy(draftPoolIds)}
                      disabled={updatingProxy}
                      className="flex-1 rounded bg-primary px-2 py-1 text-xs font-medium text-white disabled:opacity-50"
                    >
                      {updatingProxy ? "Saving..." : "Apply"}
                    </button>
                  </div>
                </div>
              )}
