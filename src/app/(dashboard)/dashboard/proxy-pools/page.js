...
export default function ProxyPoolsPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/proxy-pools/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.logs) setGenerateLogs(data.logs);
      await fetchProxyPools();
      if (data.success) {
        notify.success(`Imported ${data.imported} proxies`);
      } else {
        notify.error(data.error || "Generation failed");
      }
    } catch (error) {
      notify.error("Generation request failed");
    } finally {
      setGenerating(false);
    }
  };
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-1 sm:gap-6 sm:px-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
          ...
          <Button size="sm" variant="secondary" icon="bolt" onClick={() => setShowGenerateModal(true)}>
            Generate Proxy
          </Button>
                ...
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {Number(pool.limitHits || 0) > 0 && (
                      <Badge variant="error" size="sm" title={`Last limit hit: ${formatDateTime(pool.lastLimitAt)}`}>
                        {pool.limitHits}× limit
                      </Badge>
                    )}
      ...
      <Modal
        ...
        title="Generate Free Proxies"
        onClose={() => { if (!generating) { setShowGenerateModal(false); setGenerateLogs([]); setGenerateResult(null); } }}
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg bg-purple-500/5 border border-purple-500/10 p-3 flex flex-col gap-1.5">
            <p className="text-sm text-text-main font-medium">How it works</p>
            <p className="text-xs text-text-muted">
              Fetches free HTTP proxies from public lists, tests each against httpbin.org/ip
              (5s timeout), and imports valid ones into your proxy pools.
            </p>
            <ul className="text-xs text-text-muted list-disc pl-4 space-y-0.5">
              <li>Scrapes 4 public proxy sources (SpeedX, ShiftyTR, jetkai, roosterkid)</li>
              <li>Validates each proxy with a real HTTP request</li>
              <li>Drops dead/timeout proxies automatically</li>
              <li>Imports up to 100 valid proxies as active pool entries</li>
              <li>Proxies come from various countries (random per source)</li>
            </ul>
          </div>

          {generateLogs.length > 0 && (
            <div className="max-h-48 overflow-y-auto rounded-lg bg-black/[0.03] dark:bg-white/[0.03] p-3 font-mono text-[11px] leading-relaxed text-text-muted">
              {generateLogs.map((line, i) => (
                <div key={i}>{line}</div>
              ))}
            </div>
          )}
            ...
            <div className="rounded-lg border border-border/50 p-3 text-sm">
              {generateResult.success ? (
                <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
                  ...
                  <span>Imported <strong>{generateResult.imported}</strong> / {generateResult.valid} valid proxies</span>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-red-500">
                  <span className="material-symbols-outlined text-[18px]">error</span>
                  <span>{generateResult.error || "Generation failed"}</span>
                </div>
              )}
              <div className="mt-1 text-xs text-text-muted">
                {generateResult.totalParsed} parsed, {generateResult.valid} valid, {generateResult.imported} imported
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <Button
              fullWidth
              icon={generating ? "progress_activity" : "bolt"}
            ...
            <Button
              fullWidth
              variant="ghost"
              onClick={() => { setShowGenerateModal(false); setGenerateLogs([]); setGenerateResult(null); }}
              disabled={generating}
            >
              Close
            </Button>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 rounded-lg border border-border/50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-text-muted">
                Residential/clean IPs are tried BEFORE datacenter proxies. Mark this if the IP is not a
                public datacenter proxy (for example home ISP, mobile, or paid residential).
              </p>
            </div>
            <Toggle
              checked={formData.isResidential === true}
              onChange={() => setFormData((prev) => ({ ...prev, isResidential: !prev.isResidential }))}
              disabled={saving}
            />
          </div>
